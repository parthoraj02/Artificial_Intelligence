...
def valid_moves(x, y, room, visit_place, hurdle_stats):
    ...
    if x > 0:  # Check move up
        if room[x - 1][y] == 'H':
            hurdle_stats['hurdles'] += 1
            hurdle_stats['extra_moves'] += 1
        elif (x - 1, y) not in visit_place:
            moves.append((x - 1, y))
...
def move_towards_object(x, y, object_x, object_y, room, visit_place, hurdle_stats):
    ...
    # Determine the preferred move based on the target direction
    if object_x > x:  # Target is below
        if (x + 1, y) in possible_moves:  # Move down
            return x + 1, y
    if object_x < x:  # Target is above
        if (x - 1, y) in possible_moves:  # Move up
            return x - 1, y
    if object_y > y:  # Target is to the right
        if (x, y + 1) in possible_moves:  # Move right
            return x, y + 1
    if object_y < y:  # Target is to the left
        if (x, y - 1) in possible_moves:  # Move left
            return x, y - 1

    # If no preferred move is possible, return one of the valid moves
...
def print_results(total_objects_collected, total_move_count, total_action_count,
    ...
    print(f"Total Actions Taken: {total_action_count+total_objects_collected}")
...
def run_simulation():
    ...
    for step in range(3):
        ...
        while True:
            ...
            action_count += 1  # Increment for every action (either move or attempt to move)
    ...
    if total_objects_collected > 0:
        calculate_performance = (total_action_count+total_objects_collected) / total_objects_collected
